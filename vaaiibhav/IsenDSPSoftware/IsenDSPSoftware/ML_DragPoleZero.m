function ML_DragPoleZero
%function ML_DragPoleZero()
%
%Opens a GUI having a Unit Circle and various axes. Mouse can be used to
%move cursor in vicinity of Unit Circle and one or two poles or zeros
%(user-selectable number and type) will be created and will move with the cursor. The
%corresponding impulse response, frequency response, and z-transform will be
%displayed. Frequency response is based on the DFT of the first 256 samples
%of the impulse response generated by the pole(s) or zero(s) of the
%transfer function.
%
% Author: F. W. Isen
...
global hndFig115928 hndPoleOrZero115928 SR115928 hndOneOrTwo115928
global hndPlotPZ1 hndPlotPZ2 hndPlotZero1 hndPlotZero2 hndTxt1115928
...
hold on
hndUC115928 = plot(xs,ys,'b:');

hndCenUC115928 = plot(0,0,'b.');

hndRealAxUC115928 = plot(horlinex,vertliney,'b:');
hndImagAxUC115928 = plot(Vlinex, Vliney,'b:');

ylabel('Imaginary','fontsize',13)
xlabel('(e) Real','fontsize',13)

hndUCLabel115928 = text(0.5,-0.97,'Unit Circle','fontsize',13);
axis([-1.2 1.2 -1.15 1.15])

scnsz = get(0,'ScreenSize');
set(hndFig115928,'Position',[0.005*scnsz(3) 0.04*scnsz(4) 0.99*scnsz(3) 0.9*scnsz(4)]);
width = 0.95*scnsz(3);

posSetPoleZero115928 = [0.65*width 0.0175*scnsz(4) 0.03*width 0.035*scnsz(4)];
poslabelCheckbox = [0.68*width 0.0125*scnsz(4) 0.15*width 0.035*scnsz(4)];
uicontrol('style','text','position',poslabelCheckbox,'string','Check Box For Pole(s)','BackgroundColor',[1 1 1]);
hndPoleOrZero115928 = uicontrol(hndFig115928,'Style','checkbox','Position',posSetPoleZero115928,'backgroundcolor',[1 1 1]); 

posOneOrTwo115928 = [0.27*width 0.0175*scnsz(4) 0.03*width 0.035*scnsz(4)];
poslabelOneOrTwo = [0.28*width 0.0125*scnsz(4) 0.25*width 0.035*scnsz(4)];
uicontrol('style','text','position',poslabelOneOrTwo,'string','Check Box For Conjugate Pair','BackgroundColor',[1 1 1]);
hndOneOrTwo115928 = uicontrol(hndFig115928,'Style','checkbox','Position',posOneOrTwo115928,'backgroundcolor',[1 1 1]); 
...
hndPlotPZ1 = plot(realPZ,imagPZ,'bx','markersize',8);
hndPlotPZ2 = plot(realPZ,imagPZ,'bx','markersize',8);


hndPlotZero1 = plot(realPZ,imagPZ,'bx','markersize',8);
hndPlotZero2 = plot(realPZ,imagPZ,'bx','markersize',8);
hndTxt1115928 = text(0.06,0.06,'');
set(hndFig115928,'WindowButtonMotionFcn','DispPZCoord115928')
