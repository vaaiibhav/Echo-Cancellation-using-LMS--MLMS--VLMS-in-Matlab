function [Z,P,K,NetRp,NetAs] = LVxDesignAnalogEllipBPF(OmS1,OmP1,OmP2,OmS2,Rp,As)
% Receives analog BPF band edges in radians/sec,
% plus desired maximum Rp and minimum As and returns the zeros (Z) and
% poles (P) of an analog Elliptic BPF along with filter
% gain K and the realized values of Rp (NetRp) and As (NetAs).
% Sample calls:
% [Z,P,K,NetRp,NetAs] = LVxDesignAnalogEllipBPF(2,3,6,8,1,45)
% [Z,P,K,NetRp,NetAs] = LVxDesignAnalogEllipBPF(1,1.2,2.8,3.9,1,60)
% [Z,P,K,NetRp,NetAs] = LVxDesignAnalogEllipBPF(1,1.2,4,5,1,75)
% [Z,P,K,NetRp,NetAs] = LVxDesignAnalogEllipBPF(4,5,8,10,1,40) 
%
% Author: F. W. Isen
% Copyright 2009 by Morgan & Claypool
    ...
    [Z1,P1,K1] = LVDesignEllip(Rp,As,OmPlp,OmSlp);
    NumPoles = length(P1)
    NumZer = length(Z1)
    Num = 1; Den = 1; 
    N = [1 0 Om0Sq]; D = [0 1 0]; 
